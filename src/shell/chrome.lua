...
-- (`chicago.tui_desktop.desktop:programs`). A type that is not here is
...
-- the profile window the application named (CHICAGO_PROFILE_ENTRY):
...
-- full name, the application answers it through CHICAGO_USER_FUNC
...
-- bit rows (`chicago.shell.display:patterns`), or nil for none. The
...
function chrome.content_colors(window)
    if window.entry == "chicago.tui_desktop.desktop:window_pty" then return console_colors end
    return nil
end
...
-- ─── Start menu ──────────────────────────────────────────────────────────
--
-- The cascade layout lives in `chicago.shell.theme:menu_layout`: one pure
