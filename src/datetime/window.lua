...
function definition.view(state: any, context: any): any
    ...
    if state.tab == 1 then
        ...
        page = {kind = "column", padding = 1, children = {
            {kind = "label", size = 1, text = zone_caption(c.zone)},
            {kind = "label", text = "Часовой пояс задан системой; здесь он только показывается."},
        }}
    end
    return {kind = "column", padding = 1, gap = 0, children = {
        {kind = "tabs", id = "pages", labels = {"Дата и время", "Часовой пояс"}, active = state.tab, children = {page}},
        {kind = "row", size = 2, gap = 1, children = {
            {kind = "label", text = ""},
            {kind = "button", id = "ok", size = 10, text = "ОК", default = true},
            {kind = "button", id = "cancel", size = 10, text = "Отмена"},
            {kind = "button", id = "apply", size = 12, text = "Применить", disabled = true},
        }},
    }}
end
