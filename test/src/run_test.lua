...
local function define_tests()
    test.describe("Run", function()
        ...
        test.it("lays out the dialog on the SDK: input, OK by default, Cancel, no overlaps", function()
            ...
            test.not_nil(find_text(run_window.definition.view(state, {width = 52, height = 10}), "Введите"))
        end)
        test.it("opens Bash and Run from Start and keeps the launched shell after Run closes", function()
            ...
            local group = menu.spots["Программы"]
            test.not_nil(group)
            ...
            key(desk, "enter")
            frame = receive(frames, function(value) return find_text(value.state.ui, "Введите имя программы или команду.") ~= nil end)
