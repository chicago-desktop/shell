...
local function define_tests()
    test.describe("chicago.shell explorer sources", function()
        ...
        test.it("opens Wippy from My Computer and discovers filesystem folders inside it", function()
            local shown, err = sources.list(model.ROOT, {})
            test.is_nil(err)
            test.eq(#shown.objects, 3, "D:, the probe disk and the Control Panel")
            local control = by_id(shown.objects, model.CONTROL)
            test.not_nil(control, "the Control Panel is in My Computer")
            test.eq(control.open.path, "control")
            local disk = by_id(shown.objects, "app:disk_probe")
            test.not_nil(disk, "registry metadata discovers module-provided disks")
            test.eq(disk.open.entry, model.EXPLORER)
            test.eq(disk.open.args, model.WIPPY)
            test.eq(shown.objects[1].title, "Wippy (D:)")
            test.eq(shown.objects[1].kind, "drive")
            shown, err = sources.list(shown.objects[1].open.path, {})
            test.is_nil(err)
            test.eq(shown.title, model.WIPPY_TITLE)
            test.not_nil(by_id(shown.objects, PROBE), "the filesystem must be on D:")
            test.is_nil(by_id(shown.objects, "programs"))
            test.is_nil(by_id(shown.objects, "desktop"))
            test.is_nil(by_id(shown.objects, "windows"))
            test.is_nil(by_id(shown.objects, model.CONTROL), "the Control Panel is not on D:")
            for _, object in ipairs(shown.objects) do
                test.eq(object.kind, "folder")
        ...
        test.it("reads the drive contents with the fs module", function()
            ...
            test.eq(self_file.open.entry, "chicago.shell.viewers:notepad")
