...
Today's Notepad (`chicago.shell.viewers:notepad`) is a raw-tty viewer:
...
(`ui.file_dialog(spec)` in a new `chicago.shell.sdk:filedialog`
...
`chicago.shell.viewers:notepad` becomes an SDK application
(`pixel_render: chicago.shell.sdk:render`, `pixel_state` itself),
