...
## Окно проводника написано, но не подключено

`src/explorer/window.lua` — процесс окна «Мой компьютер»: цикл, ввод,
выделение, двойной щелчок, сетка значков, статусная строка. Записи процесса
для него нет ни в одном `_index.yaml` намеренно, поэтому на загрузку и линт
он не влияет и ни разу не поднимался. Чтобы завести:

1. запись процесса `butschster.windows.explorer:window` с
   `meta.type: tui_desktop.window`;
2. своя политика окна: `process.context`, `process.send`, `process.registry`,
   `db.get`, `registry.get`, `registry.find` — без `spawn` и `exec`;
3. `sources.list` для пути `desktop/<id>` — содержимое папки стола: модель
   такой путь возвращает, а источник его пока не читает;
4. в `defaults.lua` «Мой компьютер» ведёт на
   `butschster.tui_desktop.apps:commander`, а должен вести на это окно.

Отдельно: имя композитора у `window_api` основы зашито, поэтому окно под этой
оболочкой не дотянется до своего композитора без обхода — см. README основы.
