...
### Окна-виды: «Дата и время», калькулятор и реестр

Три программы оболочки нарисованы пикселями целиком, вместе с содержимым, —
это окна-виды по FR-005 §4б: процесса внутри нет, содержимое рисует чистая
библиотека `render` в композиторе, данные приносит процесс `state` под своим
узким актором (`butschster.windows.security:view_state` — контекст, реестр
процессов, отправить композитору; ни файлов, ни базы). Раскладка у каждого
окна — отдельная библиотека без `gfx`, и её читают оба: по ней рисуется
кнопка и по ней же поставщик считает, какая кнопка под щелчком. Две таблицы
разошлись бы на первой правке, и щелчок попадал бы на соседа.
...
увидеть. Калькулятор, реестр, диспетчер и «Выполнить…» собраны на SDK и
работают и в ячейках; окно-вид «Дата и время» в режиме без графики
открывается пустым — путь в ячейках для него не написан.
...
Синяя полоса заголовка имеет высоту 20 px — установленный владельцем
максимум. Над ней
4 px рамки; резерв округляется вверх до целых строк терминала: при высоте
ячейки от 12 до 23 px это две строки, при 24 px и выше — одна. Остаток
последней строки имеет цвет клиента.
Боковые и нижняя границы — по одной ячейке с тонкой рамкой внутри.

Кнопки заголовка рисуются общим `pixels.button` размером 18×16 px; знаки
сворачивания, разворачивания и закрытия — 10 px.
