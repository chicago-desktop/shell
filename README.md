...
## Внутри окна нельзя пользоваться библиотекой запрос-ответ

Правило короткое, а нарушение неотличимо от дефекта композитора.

Цикл ожидания ответа (`api:control` здесь, такой же в основе) забирает из
`process.inbox()` **всё подряд** и выбрасывает то, что не его ответ. В HTTP-
ручке это безвредно: её inbox больше никому не нужен. В окне через тот же
inbox приходят команды композитора — и первый же вопрос «какие окна открыты»
съедает чужое сообщение без следа.

Съеденная команда неотличима от неполученной: окно просто не отреагирует, и
искать будут в композиторе, где всё исправно.

Поэтому внутри окна ответ разбирает **собственный цикл** окна: он владеет
inbox и обрабатывает ответ наравне с остальным. Библиотека запрос-ответ
остаётся снаружи — в ручках, где ей ничего не мешает.
